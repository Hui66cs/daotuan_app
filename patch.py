...
# 3. usages of showMapView
text = text.replace(\"if (showMapView && modules.root)\", \"if (currentView === 'map' && modules.root)\")
text = text.replace(\"showMapView\", \"(currentView === 'map')\")
text = text.replace(\"setShowMapView(false)\", \"setCurrentView('editor')\")
text = text.replace(\"setShowMapView(true)\", \"setCurrentView('map')\")

# 4. navigateTo and navigateToCalendarDay
nav_logic = \"\"\"
...
# 5. renderCalendar & Map View Button
render_cal = \"\"\"
...
# 6. Map View nav buttons
header_replacement_map = \"\"\"              <button
              onClick={() => setCurrentView('editor')}
              className=\"px-5 py-2.5 rounded-xl bg-blue-600 text-white font-medium hover:bg-blue-700 transition-colors shadow-sm\"
              >
                Open Editor View
...
# 7. Editor View nav buttons
header_replacement_editor = \"\"\"            <button
              onClick={() => setCurrentView('map')}
...
print(\"Python replacement successful!\")
